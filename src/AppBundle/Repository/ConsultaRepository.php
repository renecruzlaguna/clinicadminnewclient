...
//no se usa
    ...
    //Grafic Report
    public function getTotalQueryBySpecialityForDay($speciality = -1, $date, $state) {

        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select('count(query.id)');
        $qb->from('AppBundle:Consulta', 'query');
    ...
    public function getTotalByServiceToGraphicForDay($service, $date) {
        $qb = $this->createQueryBuilder('query');
        $qb->join('query.estado', 'estado');
        $qb->join('query.factura', 'factura');
        $qb->join('factura.facturaserv', 'facturaserv');


        $qb->andWhere($qb->expr()->eq('estado', '?1'));
        $qb->setParameter(1, 4);

        $qb->join('facturaserv.servicio', 'servicio');
        $qb->andWhere($qb->expr()->eq('servicio', '?2'));
        $qb->setParameter(2, $service);
        ...
        foreach ($data as $value) {
            ...
            $serviceFacture = $value->getFactura()->getFacturaserv()->toArray();
            foreach ($serviceFacture as $serv) {
                $result += $serv->getCantidad();
            }
        }
        return $result;
    }
